...
local function checkForIcon(name)
	if file.Exists("maps/thumbs/" .. name .. ".png", "GAME") or file.Exists("maps/" .. name .. ".png", "GAME") or file.Exists("mapicon/" .. name .. ".png", "DATA") then
		return true
	else
		return false
	end
...
local function sendMapIconsToPlayer()
	...
	if GetConVar("mapicon_syncPlayerMapIconsWithServer"):GetInt() ~= 1 or sendInProgress or next(xtable2) == nil then return end
					...
					if table.Count(d) < 1 then
						...
						for e, f in pairs(d) do
							if f == nil then
								...
								if fileContent:len() >= 64000 then
									print("Error. File larger than 64kb!.." .. fileName)
...
local function savePlayerToMapList(xtable, ply)
	...
	playerToMapList[table.Count(playerToMapList) + 1] = {
		[ply] = xtable2
	}
...
hook.Add("Initialize", "mapicon_setup", function()
	...
	local files, _ = file.Find("mapicon/*.png", "DATA")
