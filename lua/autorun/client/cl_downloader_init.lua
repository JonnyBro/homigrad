if game.SinglePlayer() or not GetConVar("downloader_gui_enabled"):GetBool() then return end
	...
	for k, addonInfo in ipairs(addons) do
		...
		if IsValid(scanInfo.iconArea) then
			...
			elseif selectedValue == FILTER.SELECTED then
				if scanInfo.manual == nil and scanInfo.cachedManual == nil and scanInfo.selected == true or scanInfo.manual == nil and scanInfo.cachedManual == true or scanInfo.manual == true then
					scanInfo.iconArea:Show()
					scanInfo.iconArea:SetParent(self.addonsList)
				end
			elseif selectedValue == FILTER.IGNORED then
				if scanInfo.manual == nil and scanInfo.cachedManual == nil and scanInfo.selected == false or scanInfo.manual == nil and scanInfo.cachedManual == false or scanInfo.manual == false then
					scanInfo.iconArea:Show()
					scanInfo.iconArea:SetParent(self.addonsList)
				end
			elseif selectedValue == FILTER.MANUAL then
				if scanInfo.manual == nil and scanInfo.cachedManual ~= nil and scanInfo.cachedManual ~= scanInfo.selected or scanInfo.manual ~= nil and scanInfo.manual ~= scanInfo.selected then
					scanInfo.iconArea:Show()
					scanInfo.iconArea:SetParent(self.addonsList)
				end
			end
