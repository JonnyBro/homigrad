...
function AutoBalanceTwoTeam()
	for i = 1, #GetAll() do
		...
		if favorT then
			local ply = table.Random(team_GetPlayers(1))

			ply:SetTeam(2)
			ply:ChatPrint("Тебя перекинуло в CT команду, из-за неравенства команд.")
		else
			local ply = table.Random(team_GetPlayers(2))
...
function OpposingAllTeam()
	...
	table.CopyFromTo(team_GetPlayers(1), oldT)
	table.CopyFromTo(team_GetPlayers(2), oldCT)

	for i, ply in pairs(oldT) do
		ply:SetTeam(2)
	end

	for i, ply in pairs(oldCT) do
		ply:SetTeam(1)
	end
end