...
function CLASS:On()
	...
	for i, weapon in pairs(CLASS.weapons or empty) do
		self:Give(weapon)
	end
...
local function random_sound(name, max)
...
function CLASS:EventPoint(name, pos, radius, a1, a2)
	...
	if self:GetMoveType() ~= MOVETYPE_NOCLIP and name == "fragnade pre detonate" and tracerSee(self, pos, 52) then
		...
		timer.Simple(2, function()
			...
			EmitSound(self, random_sound("bot/yesss", 2))
		end)
	end
	...
	if name == "hitgroup killed" and a1 ~= self and a1.isContr then
		if a2.LastHitGroup == HITGROUP_HEAD then
			...
			timer.Simple(math.Rand(0.75, 1.75), function()
				...
				EmitSound(self, random_sound("bot/good_shot", 2))
			end)
		end
	end
end
...
function CLASS:Event(name, a1)
	...
	if name == "inventory close" then
		if a1.roleT then
			...
			if r == 1 then
				EmitSound(self, "bot/aah.wav")
			elseif r == 2 then
				EmitSound(self, "bot/aw_hell.wav")
			else
				EmitSound(self, "bot/aww_man.wav")
			end
		else
			if math.random(1, 2) == 2 then
				EmitSound(self, random_sound("bot/clear", 4))
			else
				EmitSound(self, random_sound("bot/he_got_away", 2))
			end
		end
	end
end
...
local sounds = {
	{
		"Right", {"bot/alright.wav", "bot/alright2.wav", "bot/yea_ok.wav"},
		{1, 1}
	},
	{
		"No", {"bot/negative.wav", "bot/negative2.wav", "bot/no.wav", "bot/no2.wav"},
		{1, 2}
	},
	{
	...
	{
		"Position", {"radio/takepoint.wav", "radio/position.wav"},
		{3, 2}
	}
}
