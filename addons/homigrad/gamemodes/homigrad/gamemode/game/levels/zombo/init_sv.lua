		...
		for i, ply in pairs(team.GetPlayers(2)) do
			...
			for i, point in pairs(list) do
				if ply:GetPos():Distance(point[1]) < (point[3] or 250) then
					...
					PrintMessage(3, "Выживший сбежал, осталось " .. (CTAlive - 1) .. " выживших")
				end
			end
		end
	end
...
function zombo.EndRound(winner)
...
function zombo.PlayerInitialSpawn(ply)
	ply:SetTeam(2)
end
...
function zombo.PlayerDeath(ply, inf, att)
	return false
end
