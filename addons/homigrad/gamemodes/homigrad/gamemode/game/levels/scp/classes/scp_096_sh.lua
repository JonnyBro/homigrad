...
if CLIENT then
	...
	hook.Add("CalcAddFOV", "SCP096", function()
		...
		ADDROLL = ADDROLL + math.cos(CurTime() / 10) * 15 * Lerp
	end)
	...
	hook.Add("HUDPaint", "SCP096", function()
		...
		draw.GradientUp(0, 0, w, h / 4 * Lerp)
		local size = h / 3 * Lerp

		draw.GradientUp(0, 0, w, size)
		draw.GradientDown(0, h - size, w, size + 1)
		draw.GradientUp(0, 0, w, size * 2)
		draw.GradientDown(0, h - size, w, size * 2 + 1)

		surface.SetDrawColor(0, 0, 0, Lerp * 200)

		for i = 1, math.random(4, 5) do
			surface.SetBG("points" .. math.random(4, 6) .. "0")
			local x, y = math.random(0, w), math.random(0, h)
			draw.BGScale(-x, -y, w + x, h + y, math.Clamp(math.abs(math.cos(time * 10 + math.Rand(-1, 1))) * 25, 10, 25))
		end
	end)
	...
	function CLASS.GlobalThink(list)
		if (scp096Here or 0) > 0 then
			...
			if IsValid(CLASS.ScaredSound) then
				CLASS.ScaredSound:FadeOut(2)
			end
		end
...
function CLASS.Looking(self)
	...
	for i, ply in pairs(player.GetAll()) do
		...
		if CanLook(self, ply:EyePos()) then
...
function CLASS:Think()
	...
	if oldState ~= state then
		...
		if SERVER and state == 0 then
			for k in pairs(self.playersView) do
				self.playersView[k] = nil
			end
		end
	end
	...
	if state == 0 then
		if CLIENT then
			...
			if self:GetNWFloat("Panic", 0) > CurTime() then
				...
				if not isClient and snd.dsp == 0 then
					scp096Here = snd.volumeTrue
				end
			end
		end
	elseif state == 1 then
		...
		if CLIENT then
			...
			local k = 1 - math.max(angryStart + angryDelay - CurTime(), 0) / angryDelay

			snd.dis = 8000 * math.max(k, 0.2)
			snd.alwaysView = true

			if not isClient and scp096Volume < snd.volumeTrue and snd.dsp == 0 then
				scp096Volume = snd.volumeTrue
			end
		else
			...
			if (self.delaySequencePanic or 0) < CurTime() then
				...
				self:SetNWInt("Sequence", math.random(1, 6))
			end
		end
	elseif state == 2 then
		if CLIENT then
				...
				for i, ent in pairs(ents.FindInSphere(self:GetPos(), killDis)) do
					if not ent:GetNoDraw() and JMod.IsDoor(ent) then
						...
						if ent.hpDoor <= 0 then
							...
							sound.Emit(ent, "physics/metal/metal_grate_impact_hard" .. math.random(1, 3) .. ".wav", 120, 90)
							sound.Emit(ent, "physics/metal/metal_sheet_impact_hard" .. math.random(6, 7) .. ".wav", 120, 90)
						end
					end
				end
			end
...
function CLASS:GuiltLogic(ply)
	return false
end

function CLASS:ShouldFake()
	return false
end

function CLASS:ShouldUpWeapon()
	return self:GetMoveType() == MOVETYPE_NOCLIP
end

function CLASS:ShouldUpItem()
	return self:GetMoveType() == MOVETYPE_NOCLIP
end

function CLASS:JModArmorEquip()
	return false
end
