...
function CLASS:Off()
	...
	if IsValid(self.FootstepSound) then
		self.FootstepSound:Remove()
	end
end
...
function CLASS.CanLook(ply, pos)
	...
	tr.filter = function(ent) return ent == ply or (not ent:IsPlayer() and not ent:IsNPC() and not ent:IsRagdoll()) end
	tr.mask = MASK_SHOT
...
if CLIENT then
	...
	hook.Add("HUDPaint", "SCP173", function()
		...
		if hold then
			...
			k = math.max(start + delay - time, 0) / delay
			k2 = (not noblink and math.max((start + blinkTime - time) / blinkTime, 0) * 3) or 0
		end
		...
		DrawRect(0, 0, w, h * k2)
		draw.GradientUp(0, h * k2, w, h / 3)

		local kk = math.abs(math.cos(CurTime() * 2))

		SetDrawColor(0, 0, 0, 25 * kk)
		draw.GradientUp(0, 0, w, h / 4 * kk)
		...
		for i = 1, math.random(4, 5) do
			...
			draw.BGScale(-x, -y, w + x, h + y, math.Clamp(math.abs(math.cos(time * 10 + math.Rand(-1, 1))) * 25, 10, 10))
		end
	end)
...
CLASS.horrorSounds = {"sfx/horror/horror1.mp3", "sfx/horror/horror10.mp3", "sfx/horror/horror2.mp3", "sfx/horror/horror14.mp3", "sfx/horror/horror9.mp3"}
...
function CLASS:Think()
			...
			if self.canMove then
				if self:KeyDown(IN_ATTACK) then
					...
					for i, ply in pairs(player.GetAll()) do
						...
						if util.TraceLine(tr).Entity ~= ent then continue end
	...
	if CLIENT then
		...
		vel = (math.abs(vel[1]) + math.abs(vel[2])) / 2
