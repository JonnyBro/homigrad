...
function bodycam.GetListMul(lst, mul, func, max)
	local newList = {}

	mul = math.Round(#lst * mul)
	...
	for i = 1, mul do
		local ply, key = table.Random(lst)
		lst[key] = nil
...
function bodycam.Think()
	...
	if CurTime() >= bodycam.LastWave then
		...
		for _, v in pairs(player.GetAll()) do
			...
			for i, lst in pairs(players) do
				bahmut.SelectRandomPlayers(lst[1], 6, bahmut.GiveAidPhone)
				bahmut.SelectRandomPlayers(lst[2], 6, bahmut.GiveAidPhone)
			end
		end
...
function bodycam.GetCountLive(lst, func)
	local count = 0
	local result
...
function bodycam.GiveSwep(ply, lst, mulClip1)
	if not lst then return end
	local wep = ply:Give(type(lst) == "table" and lst[math.random(#lst)] or lst)
...
function bodycam.PlayerInitialSpawn(ply)
...
function bodycam.CanRandomNext()
	return false
end