...
function deathrun.EndRound(winner)
...
function deathrun.PlayerInitialSpawn(ply)
	ply:SetTeam(2)
end
...
function deathrun.PlayerDeath(ply, inf, att)
	return false
end
...
function deathrun.NoSelectRandom()
	local a = string.find(string.lower(game.GetMap()), "deathrun")
