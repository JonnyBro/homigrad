...
net.Receive("ply_take_item",function(len,ply)
	...
	local lootEnt = net.ReadEntity()
	if not IsValid(lootEnt) then return end
    if lootEnt.fake == nil and not lootEnt:GetClass() == "box_normal" then ply:Kick("Exploiter FOUND!") return end
    if ply:GetPos():Distance(lootEnt:GetPos()) >= 120 then ply:Kick("Exploiter FOUND!") return end
