...
hook.Add("PlayerSpawn", "hg_inventorysync", function(lootEnt)
...
hook.Add("Player Think", "Looting", function(ply)
	...
	if not ply.fake and ply:Alive() then
		if ply.okeloot ~= key and key then
			...
			if IsValid(hitEnt) and hitEnt.IsJModArmor then
				hitEnt = hitEnt.Owner
			end
			...
			hitEnt:CallOnRemove("fuckoff", function()
				send(nil, hitEnt, true)
			end)
		end
	end
...
net.Receive("ply_take_item", function(len, ply)
	...
	if lootEnt.fake == nil and lootEnt:GetClass() ~= "box_normal" then return ply:Kick("Exploiter FOUND!") end
	if ply:GetPos():Distance(lootEnt:GetPos()) >= 120 then return ply:Kick("Exploiter FOUND!") end
	...
	if adminOnlyWeps[wep] and not ply:IsAdmin() then return ply:Kick() end
