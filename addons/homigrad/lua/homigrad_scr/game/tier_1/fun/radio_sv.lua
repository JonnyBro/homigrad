...
COMMANDS.radio = {
	function(ply, args)
		...
		if group ~= "Sponsor" or group ~= "MegaSponsor" or not ply:IsAdmin() then return end
...
hook.Add("EntityTakeDamage", "radio_nodmg", function(radio, dmginfo)
	if radio.radio and not radio.destructible then
		...
		if IsValid(att) and att:IsPlayer() and not att:IsAdmin() and math.random(1, 13) == 13 then
			att:ChatPrint("Не трогай радио.")
		end
