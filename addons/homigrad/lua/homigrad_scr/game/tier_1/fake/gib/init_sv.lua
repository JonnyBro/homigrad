...
function Gib_Input(rag, bone, dmgInfo)
	...
	if dmgInfo:GetDamage() >= 50 and dmgInfo:IsDamageType(DMG_BLAST) and not gibRemove[phys_bone] then
		...
		for bonename in pairs(validBone) do
			...
			if rag:GetBonePosition(bone):Distance(dmgPos) <= 75 then
				access = true
				break
			end
		end
...
hook.Add("EntityTakeDamage", "Gib", function(ent, dmgInfo)
	...
	if phys_bone == 0 then return end
	...
	for ent in pairs(gib_ragdols) do
		...
		if ent.BloodGibs and ent.Blood > 0 then
			...
			if (ent.BloodNext or 0) < time then
				ent.BloodNext = time + math.Rand(0.25, 0.5) / math.max(k, 0.25)
				ent.Blood = math.max(ent.Blood - 25, 0)

				tr.start = ent:GetPos()
				tr.endpos = tr.start + Vector(math.Rand(-1, 1), math.Rand(-1, 1), -math.Rand(0.25, 0.4)) * 125 * math.Rand(0.8, 1.2)
				tr.filter = ent
			...
			for phys_bone, phys_obj in pairs(ent.gibRemove) do
				if (BloodGibs[phys_bone] or 0) < time then
					...
					BloodGibs[phys_bone] = time + math.Rand(0.07, 0.1) / math.max(k, 0.25)

					BloodParticle(phys_obj:GetPos(), phys_obj:GetVelocity() + phys_obj:GetAngles():Forward() * math.Rand(200, 250) * k)
				end
			end
		end
	end
