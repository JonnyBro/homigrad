...
hook.Add("DoPlayerDeath", "blad", function(ply, att, dmginfo)
	...
	if IsValid(ent) then
		ent:SetNWEntity("RagdollOwner", nil)
	end
...
concommand.Add("fake", function(ply)
	if timer.Exists("faketimer" .. ply:EntIndex()) then return end
	if timer.Exists("StunTime" .. ply:EntIndex()) then return end
	if ply:GetNWEntity("Ragdoll").isheld == true then return end
	if ply.Seizure then return end
	if ply.brokenspine then return end
...
function PlayerMeta:CreateRagdoll(attacker, dmginfo, force)
	...
	rag:CallOnRemove("huyhjuy", function()
		self.firstrag = false
	end)
...
hook.Add("OnPlayerHitGround", "hg_hitgroundfake", function(ply, a, b, speed)
	if speed > 200 then
...
hook.Add("StartCommand", "hg_alwaysduckinfake", function(ply, cmd)
	local rag = ply:GetNWEntity("Ragdoll")

	if (ply.GotUp or 0) - CurTime() > -0.1 and not IsValid(rag) then
		cmd:AddKey(IN_DUCK)
	end
...
hook.Add("Player Think", "InternalBleeding", function(ply, time)
	for i, ply in pairs(player.GetAll()) do
		...
		if ply.OrgansNextThink < CurTime() then
...
hook.Add("PlayerSay", "dropweaponhuy", function(ply, text)
	if string.lower(text) == "*drop" or string.lower(text) == "/drop" or string.lower(text) == "*dropweapon" or string.lower(text) == "/dropweapon" then
		if not ply.fake then
			ply:DropWeapon1()
...
hook.Add("UpdateAnimation", "hg_updateAnims", function(ply, event, data)
