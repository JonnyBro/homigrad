...
-- Вызов функции при спавне игрока
	...
	if not IsValid(ply.wep) and table.HasValue(Guns, ply.curweapon) then
		...
		if IsValid(rag) then
			...
			if TwoHandedOrNo[ply.curweapon] then
				...
				local buggedModelIDK = ply:GetModel() == "models/knyaje pack/dibil/sso_politepeople.mdl"

				rag:GetPhysicsObjectNum(rag:TranslateBoneToPhysBone(rag:LookupBone("ValveBiped.Bip01_L_Hand"))):SetAngles(ply:GetNWEntity("Ragdoll"):GetPhysicsObjectNum(7):GetAngles() - Angle(0, 0, buggedModelIDK and 90 or 180))
...
function FireShot(wep)
	...
	function wep:RicochetOrPenetrate(initialTrace)
		...
		elseif ApproachAngle < (MaxRicAngle * .75) then -- Ricochet
