	...
	if not rag or (rag and not dmginfo:IsDamageType(DMG_CRUSH)) then
		...
		if hitgroup == HITGROUP_HEAD and math.random(1, math.max(math.floor(armorDur), 1)) == 1 and dmginfo:IsDamageType(DMG_BULLET + DMG_SLASH + DMG_CLUB + DMG_GENERIC + DMG_BUCKSHOT) then
			timer.Simple(0.01, function()
				...
				if ply:Alive() and not ply.fake and not ply.nopain and (IsValid(wep) and not wep.GetBlocking and true or not wep:GetBlocking()) then
					Faking(ply)
				end
			end)
		end
		...
		if huy then
			if ply.Organs["brain"] ~= 0 and dmginfo:IsDamageType(DMG_BULLET) and not inf.RubberBullets then
				...
				if ply.Organs["brain"] == 0 then return ply:Kill() end
			end
		end
		...
		if huy then
			if ply.Organs["liver"] ~= 0 and not dmginfo:IsDamageType(DMG_CLUB) then
				...
				if ply.Organs["liver"] == 0 then
					ply:ChatPrint("Ты чувствуешь очень сильную боль в области печени.")
				end
			end
		end
		...
		if huy then
			if ply.Organs["stomach"] ~= 0 and not dmginfo:IsDamageType(DMG_CLUB) then
				...
				if ply.Organs["stomach"] == 0 then
					ply:ChatPrint("Ты чувствуешь очень сильную боль в животе.")
				end
			end
		end
		...
		if huy then
			if ply.Organs["intestines"] ~= 0 and not dmginfo:IsDamageType(DMG_CLUB) then
				...
				if ply.Organs["intestines"] == 0 then
					ply:ChatPrint("Ты чувствуешь очень сильную боль в области кишечника.")
				end
			end
		end
		...
		if huy then
			if ply.Organs["heart"] ~= 0 and not dmginfo:IsDamageType(DMG_CLUB) then
				...
				if ply.Organs["heart"] == 0 then
					ply:ChatPrint("Ты чувствуешь очень сильную боль в сердце.")
				end
			end
		end
		...
		if huy or huy2 then
			if ply.Organs["spine"] ~= 0 then
				...
				if ply.Organs["spine"] == 0 then
					...
					ply:ChatPrint("Произошёл перелом спины.")
