...
if SERVER then
	...
	local function CanUseSpawnMenu(ply, class)
		...
		if allowedgroups[ply:GetUserGroup()] or GetGlobalBool("AccessSpawn") then return true end
	...
	hook.Add("PlayerSpawnedVehicle", "sv_round", function(ply, model, ent)
		spawn(ply, "vehicle", ent)
	end)

	hook.Add("PlayerSpawnedRagdoll", "sv_round", function(ply, model, ent)
		spawn(ply, "ragdoll", ent)
	end)

	hook.Add("PlayerSpawnedEffect", "sv_round", function(ply, model, ent)
		spawn(ply, "effect", ent)
	end)

	hook.Add("PlayerSpawnedProp", "sv_round", function(ply, model, ent)
		spawn(ply, "prop", ent)
	end)

	hook.Add("PlayerSpawnedSENT", "sv_round", function(ply, model, ent)
		spawn(ply, "sent", ent)
	end)

	hook.Add("PlayerSpawnedNPC", "sv_round", function(ply, model, ent)
		spawn(ply, "npc", ent)
	end)

	hook.Add("CanTool", "sv_round", function(ply, tr, toolname, tool, button) return CanUseSpawnMenu(ply, "tool", toolname, tr) end)
else
