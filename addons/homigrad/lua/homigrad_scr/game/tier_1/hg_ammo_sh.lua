...
if CLIENT then
	function AmmoMenu(ply)
		...
		DermaNumSlider:SetText("Кол-во сброса")
		...
		DLabel:SetText("ЛКМ - Скинуть указанное кол-во\nПКМ - Скинуть все")
...
if SERVER then
	...
	net.Receive("drop_ammo", function(len, ply)
		...
		if ply:GetAmmoCount(ammotype) - count < 0 then
			ply:ChatPrint("У тебя столько нет патронов")

			return
		end

		if count < 1 then
			ply:ChatPrint("Ноль патронов не скинуть")

			return
		end

		if not ammolistent[ammotype] then
			ply:ChatPrint("Нет ентити этих патрон...")
