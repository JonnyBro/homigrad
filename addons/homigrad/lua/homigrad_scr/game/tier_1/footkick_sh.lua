...
if CLIENT then
	...
	local function KickThink()
		for k, v in pairs(player.GetAll()) do
			...
			if GetViewEntity() == v and (not v.ShouldDrawLocalPlayer or not v:ShouldDrawLocalPlayer()) and Kicking and v.StopKick and v.StopKick > CurTime() then
				local off = Vector(kickvmoffset.x, kickvmoffset.y, kickvmoffset.z)
				off:Rotate(CalcPlayerModelsAngle(v))

				if not IsValid(v.CreateLegs) then
					-- Creating Main Leg
				...
				if not IsValid(v.CreatePMLegs) then
					-- Creating PM Leg
...
if SERVER then
	...
	function BSModKick(ply)
		if not ply:Alive() or ply.fake then return false end
