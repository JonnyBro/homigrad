...
local prekols = {"Get a job", "Get a life", "hg_disable_stoprenderunfocus 1 чтобы выключить эти смешнявки", "ураааа, ты свернулся... Потрогай траву, играть вечность плохо.", "kys"}
...
local deathtrack = {"https://cdn.discordapp.com/attachments/1144224221334097974/1144224389970272388/death1.mp3", "https://cdn.discordapp.com/attachments/1144224221334097974/1144226357967065180/death2.mp3", "https://cdn.discordapp.com/attachments/1144224221334097974/1144230250465734797/death3.mp3", "https://cdn.discordapp.com/attachments/1144224221334097974/1144238942862979142/death4.mp3",}

local angZero = Angle(0, 0, 0)
local playing = false

local deathtexts = {"ТЫ МЁРТВ", "ПОХОЖЕ, ТЫ СДОХ", "ПОТРАЧЕНО", "ВАУ, ТЫ УМЕР", "ЖИЗНЬ ЗАКОНЧЕНА", "GAME OVER", "WASTED", "МЁРТВ", "ПОМЕР", "ТРУПАК", "МЕРТВЕЦ", "СДОХ", "ТВОЯ ОСТОНОВКА", "ВРЕМЯ ВЫШЛО", "МИССИЯ ПРОВАЛЕНА", "ВОТ И ВСЕ!", "КОНЕЦ", "FILINA?", "DEAD", "TRY AGAIN"}
...
function CalcView(ply, vec, ang, fov, znear, zfar)
	...
	if wep and weps[wep:GetClass()] then
		...
		if weaponClass == "weapon_glock18" then
			--Vector(3.85,10,1.45)
			vecWep = hand.Pos + hand.Ang:Up() * 3.5 - hand.Ang:Forward() * 10 + hand.Ang:Right() * 0.35
		...
		if weaponClass == "weapon_minu14" then
			--Vector(5,4,0.95)
			vecWep = hand.Pos + hand.Ang:Up() * 5 - hand.Ang:Forward() * 4 + hand.Ang:Right() * 1
			angWep = hand.Ang + Angle(0, 0, 0)
		end
		...
		if weaponClass == "weapon_ump" then
			--Vector(6.6,7,1.35)
			vecWep = hand.Pos + hand.Ang:Up() * 4.5 - hand.Ang:Forward() * 9 + hand.Ang:Right() * 0.8
			angWep = hand.Ang + Angle(0, 0, 0)
		end
		...
		if weaponClass == "weapon_hk_arbalet" then
			--Vector(2.5,10,0.3)
			vecWep = hand.Pos + hand.Ang:Up() * 8.3 - hand.Ang:Forward() * 8 + hand.Ang:Right() * 1.3
			angWep = hand.Ang + Angle(20, 0, 0)
		end
...
hook.Add("RenderScreenspaceEffects", "BloomEffect-homigrad", function()
	if GetConVar("hg_bodycam"):GetInt() == 1 and LocalPlayer():Alive() then
		...
		draw.Text({
			text = "AXON BODY " .. randomnum .. " XG8A754GH",
			font = "BodyCamFont",
			pos = {ScrW() - 650, 100}
		})
