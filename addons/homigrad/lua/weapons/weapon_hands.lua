...
SWEP.Contact = ""
SWEP.Purpose = ""
SWEP.Instructions = "Ваши руки\nЛКМ/R: поднять/опустить кулаки;\n- В поднятом состоянии: ЛКМ - Удар, ПКМ - Блок;\n- В опущенном состоянии: E - Открыть инвентарь игрока на которого смотрите, R - Проверить пульс, ЛКМ - Провести СЛР;\n- При удержании предмета: R - зафиксировать предмет в воздухе, E - крутить предмет в воздухе."
	...
	if IsValid(phys) then
		...
		if self.CarryEnt:GetClass() == "prop_ragdoll" then
			...
			if self:GetOwner():KeyPressed(IN_RELOAD) then
				if not ply then
					self:GetOwner():ChatPrint("Он мёртв...")
				else
...
hook.Add("ShouldCollide", "hjuyhhy", function(ent1, ent2) end)
