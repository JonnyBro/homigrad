...
if SERVER then
	...
	function dayofcodingHUD()
		if not IsValid(HappyMusic) then return end
		...
		if HappyMusic:GetState() == GMOD_CHANNEL_STOPPED then
			HappyMusic:Stop()
		end
	end
...
local items = {
	medkit = {"https://cdn.discordapp.com/attachments/1106617550227374154/1111271030166671400/Catbycat_Maxwell_the_Cat_Theme_www.lightaudio.ru.mp3", "models/w_models/weapons/w_eq_medkit.mdl", 13, nil, Angle(-30, 0, 0)},
	granade = {"https://cdn.discordapp.com/attachments/1106617550227374154/1111277143863869480/19._DOOM_www.lightaudio.ru.mp3", "models/weapons/w_jj_fraggrenade.mdl", 70, nil, Angle(-30, 0, 0)},
	murder = {"https://cdn.discordapp.com/attachments/1106617550227374154/1111277916362395748/c1141400eb9ca60.mp3", "models/player/mkx_jajon.mdl", 2, Vector(0, 0, -40), Angle(-40, 0, 0),},
	burger = {"https://cdn.discordapp.com/attachments/1106617550227374154/1111278180934897737/Face_-_BURGER_58015745.mp3", "models/foodnhouseholditems/mcdburgerbox.mdl", 15, nil, Angle(-30, 0, 0)},
	sex = {"https://cdn.discordapp.com/attachments/1106617550227374154/1111282767670546463/zvuki-orgazm.mp3", "models/props_c17/oildrum001_explosive.mdl", 7},
	glasses = {"https://cdn.discordapp.com/attachments/1106617550227374154/1111287983975432242/kizaru_-_Zerkalo_75988545.mp3", "models/gmod_tower/aviators.mdl", false, nil, Angle(-30, 0, 0)},
	romantical = {"https://cdn.discordapp.com/attachments/1062836529531211776/1116039322773233704/A_Haunted_House_2_Mark_Henry_scene_good_version_HD.mp3", "models/Humans/Group01/male_03.mdl", false, Vector(0, 0, -40), Angle(-40, 0, 0),},
	bow = {"https://cdn.discordapp.com/attachments/1100836886047621190/1111941186614218752/-_1.mp3", "models/weapons/w_snij_awp.mdl", 70, nil, Angle(-40, 0, 0)},
	nuck = {"https://cdn.discordapp.com/attachments/1109483230966599730/1121490477519228979/Shadow_Wizard_Money_Gang_EXTENDED.mp3", "models/chappi/mininuq.mdl", false, Vector(-15, -15, 10), Angle(-40, 0, 0)},
}
...
if SERVER then
		...
		function(ply, args)
			...
			elseif args[1] == "^" then
				...
				for i, ply in pairs(player.GetAll()) do
					if string.find(ply:Nick(), args[1]) then
						send(args[2], ply)
					end
				end
			end
	...
	local function stop()
		...
		if IsValid(Item_Model) then
			Item_Model:Remove()
		end

		if IsValid(Item_Station) then
			Item_Station:Stop()
		end
	end
	...
	net.Receive("the item!", function()
		...
		sound.PlayURL(url, "mono", function(_station)
			...
			hook.Add("HUDPaint", "the item!", function()
				...
				if Item_Station:GetState() == GMOD_CHANNEL_STOPPED then
					stop()
				end
			end)
		end)
