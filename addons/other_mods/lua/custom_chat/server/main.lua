...
sql.Query("CREATE TABLE IF NOT EXISTS " .. LAST_SEEN_TABLE .. " ( SteamID TEXT NOT NULL PRIMARY KEY, LastSeen INTEGER NOT NULL );")

function CustomChat:GetLastSeen(steamId)
	local row = sql.QueryRow("SELECT LastSeen FROM " .. LAST_SEEN_TABLE .. " WHERE SteamID = '" .. steamId .. "';")

	if row and row.LastSeen then return row.LastSeen end
end
...
function CustomChat:SetLastSeen(steamId, time)
	...
	if row then
		local success = sql.Query("UPDATE " .. LAST_SEEN_TABLE .. " SET LastSeen = " .. time .. " WHERE SteamID = '" .. steamId .. "';")

		if success == false then
			CustomChat.Print("SetLastSeen SQL for player %s failed: %s", steamId, sql.LastError())
		end
	else
		local success = sql.Query("INSERT INTO " .. LAST_SEEN_TABLE .. " ( SteamID, LastSeen ) VALUES ( '" .. steamId .. "', " .. time .. " );")
