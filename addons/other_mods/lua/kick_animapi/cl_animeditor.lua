...
surface.CreateFont("DefaultFontVerySmall", {
	font = "tahoma",
	size = 10,
	weight = 0,
	antialias = false
...
surface.CreateFont("DefaultFontSmallDropShadow", {
	font = "tahoma",
	size = 11,
	weight = 0,
	shadow = true,
...
surface.CreateFont("DefaultFontBold", {
	font = "tahoma",
	size = 13,
	weight = 1000,
	antialias = false
...
surface.CreateFont("DefaultFontLarge", {
	font = "tahoma",
	size = 16,
	weight = 0,
	antialias = false
...
boneList["ValveBiped.Bip01"] = {"ValveBiped.Bip01_Pelvis", "ValveBiped.Bip01_Spine", "ValveBiped.Bip01_Spine1", "ValveBiped.Bip01_Spine2", "ValveBiped.Bip01_Spine4", "ValveBiped.Bip01_Neck1", "ValveBiped.Bip01_Head1", "ValveBiped.Bip01_R_Clavicle", "ValveBiped.Bip01_R_UpperArm", "ValveBiped.Bip01_R_Forearm", "ValveBiped.Bip01_R_Hand", "ValveBiped.Bip01_L_Clavicle", "ValveBiped.Bip01_L_UpperArm", "ValveBiped.Bip01_L_Forearm", "ValveBiped.Bip01_L_Hand", "ValveBiped.Bip01_R_Thigh", "ValveBiped.Bip01_R_Calf", "ValveBiped.Bip01_R_Foot", "ValveBiped.Bip01_R_Toe0", "ValveBiped.Bip01_L_Thigh", "ValveBiped.Bip01_L_Calf", "ValveBiped.Bip01_L_Foot", "ValveBiped.Bip01_L_Toe0"}

boneList["ValveBiped"] = {"ValveBiped.hips", "ValveBiped.leg_bone1_L", "ValveBiped.leg_bone2_L", "ValveBiped.leg_bone3_L", "ValveBiped.Bip01_L_Foot", "ValveBiped.Bip01_L_Toe0", "ValveBiped.leg_bone1_R", "ValveBiped.leg_bone2_R", "ValveBiped.leg_bone3_R", "ValveBiped.Bip01_R_Foot", "ValveBiped.Bip01_R_Toe0", "ValveBiped.spine1", "ValveBiped.spine2", "ValveBiped.spine3", "ValveBiped.spine4", "ValveBiped.neck1", "ValveBiped.neck2", "ValveBiped.head", "ValveBiped.clavical_L", "ValveBiped.arm1_L", "ValveBiped.arm2_L", "ValveBiped.hand1_L", "ValveBiped.clavical_R", "ValveBiped.arm1_R", "ValveBiped.arm2_R", "ValveBiped.hand1_R", "ValveBiped.bone", "ValveBiped.bone1", "ValveBiped.bone2"}
...
local function PaintTopBar()
	...
	if selectedBone and selectedBone ~= "" then
		...
		if boneID then
			...
			if not leftDown and draggingDir then
				draggingDir = nil
			end
			...
			if (dist < 10 and not draggingDir) or draggingDir == "RR" then
			...
			if (dist < 10 and not draggingDir) or draggingDir == "RU" then
				surface.SetDrawColor(255, 255, 255, 255)
				draggingDir = "RU"
			...
			if (dist < 10 and not draggingDir) or draggingDir == "RF" then
				surface.SetDrawColor(255, 255, 255, 255)
				draggingDir = "RF"
...
local function NewAnimation()
	...
	form:Help([[Gestures are keyframed animations that use the current position and angles of the bones. They play once and then stop automatically.
	...
	timer.Simple(0.01, function()
		frame:SetSize(form:GetWide() + 10, 450)
		frame:Center()
	end)
end
...
local function LoadAnimation()
	...
	for i, v in pairs(GetLuaAnimations()) do
		--anim editor uses this internally
		if i ~= "editortest" and i ~= "editingAnim" and not string.find(i, "subPosture_") then
			box:AddChoice(i)
		end
	end
...
local function SaveAnimation()
	...
	Derma_StringRequest("Question", "Save as...", animName or "", function(strTextOut)
		RegisterLuaAnimation(strTextOut, animationData)
		file.Write("animations/" .. strTextOut .. ".txt", util.TableToJSON(animationData))
	end, function(strTextOut) end, "Save", "Cancel")
end
...
local function FixMouse()
	...
	local notOverPanel = not IsMouseOverPanel()

	if not input.IsMouseDown(MOUSE_RIGHT) and rightDown then
		rightDown = false
	elseif input.IsMouseDown(MOUSE_RIGHT) and not rightDown and notOverPanel then
		rightDown = true
		pressedPos[1] = gui.MouseX()
		pressedPos[2] = gui.MouseY()
	elseif input.IsMouseDown(MOUSE_RIGHT) and rightDown then
	...
	if input.IsMouseDown(MOUSE_LEFT) and not leftDown and notOverPanel then
		if draggingDir then
			pressedPos[1] = gui.MouseX()
			pressedPos[2] = gui.MouseY()
			leftDown = true
		end
	elseif not input.IsMouseDown(MOUSE_LEFT) and leftDown then
		leftDown = false
	elseif input.IsMouseDown(MOUSE_LEFT) and leftDown then
		local mvmtX = gui.MouseX() - pressedPos[1]
...
local function AnimationEditorOn()
	...
	timer.Simple(0.01, function()
		frame:SetPos(ScrW() - 200, ScrH() - mainSettings:GetTall() - timeLine:GetTall() * 1.7)
	end)
...
function MAIN:Init()
	...
	viewcode.DoClick = function()
		local str = OutputCode()
		if not str then return end
	...
	distSlider.OnValueChanged = function(s, v)
		camDist = v
	end
	...
	boneSet.OnSelect = function(s, i, v, d)
		selectedBoneSet = v
		self:RefreshBoneSet()
	end
...
--get restart pos in seconds
function TIMELINE:ResolveRestart()
	firstPass = false
	local timeInSeconds = 0
...
--get restart pos in seconds
function TIMELINE:ResolveStart()
	firstPass = true
	local timeInSeconds = 0
...
function TIMELINE:AddKeyFrame()
	...
	--postures have only one keyframe
	if animType == TYPE_POSTURE then
		self.addKeyButton:SetDisabled(true)
	end
...
function KEYFRAME:Init()
	...
	if addFrame then
		self.AnimationKeyIndex = table.insert(animationData.FrameData, {
			FrameRate = 1,
			BoneInfo = {}
		})
...
function KEYFRAME:OnMousePressed(mc)
	...
	elseif mc == MOUSE_RIGHT then
		...
		menu:AddOption("Change Frame Length", function()
			Derma_StringRequest("Question", "How long should this frame be (seconds)?", "1.0", function(strTextOut)
				self:SetLength(tonumber(strTextOut))
			end, function(strTextOut) end, "Set Length", "Cancel")
		end)

		menu:AddOption("Change Frame Rate", function()
			Derma_StringRequest("Question", "Set frame " .. self:GetAnimationIndex() .. "'s framerate", "1.0", function(strTextOut)
				self:SetLength(1 / tonumber(strTextOut))
			end, function(strTextOut) end, "Set Frame Rate", "Cancel")
		end)
...
function SLIDERS:Init()
	...
	self.Sliders.MU.OnValueChanged = function(s, v)
		self:OnSliderChanged("MU", v)
	end

	self.Sliders.MU.Label:SetTextColor(Color(0, 0, 255, 255))
	local oldEnter = self.Sliders.MU.Wang.OnEnter

	self.Sliders.MU.Wang.OnEnter = function(s)
		self:OnSliderChanged("MU", self.Sliders.MU.Wang:GetValue())
		self.Sliders.MU.Slider:InvalidateLayout()
		oldEnter(s)
	end

	self.Sliders.MR = self:NumSlider("Translate RIGHT", nil, -100, 100, 0)

	self.Sliders.MR.OnValueChanged = function(s, v)
		self:OnSliderChanged("MR", v)
	end

	self.Sliders.MR.Label:SetTextColor(Color(255, 0, 0, 255))

	self.Sliders.MR.Wang.OnEnter = function(s)
		self:OnSliderChanged("MR", self.Sliders.MR.Wang:GetValue())
		self.Sliders.MR.Slider:InvalidateLayout()
		oldEnter(s)
	end

	self.Sliders.MF = self:NumSlider("Translate FORWARD", nil, -100, 100, 0)

	self.Sliders.MF.OnValueChanged = function(s, v)
		self:OnSliderChanged("MF", v)
	end

	self.Sliders.MF.Label:SetTextColor(Color(0, 255, 0, 255))

	self.Sliders.MF.Wang.OnEnter = function(s)
		self:OnSliderChanged("MF", self.Sliders.MF.Wang:GetValue())
		self.Sliders.MF.Slider:InvalidateLayout()
		oldEnter(s)
	end

	self.Sliders.RU = self:NumSlider("Rotate UP", nil, -360, 360, 0)

	self.Sliders.RU.OnValueChanged = function(s, v)
		self:OnSliderChanged("RU", v)
	end

	self.Sliders.RU.Label:SetTextColor(Color(0, 255, 0, 255))

	self.Sliders.RU.Wang.OnEnter = function(s)
		self:OnSliderChanged("RU", self.Sliders.RU.Wang:GetValue())
		self.Sliders.RU.Slider:InvalidateLayout()
		oldEnter(s)
	end

	self.Sliders.RR = self:NumSlider("Rotate RIGHT", nil, -360, 360, 0)

	self.Sliders.RR.OnValueChanged = function(s, v)
		self:OnSliderChanged("RR", v)
	end

	self.Sliders.RR.Label:SetTextColor(Color(255, 0, 0, 255))

	self.Sliders.RR.Wang.OnEnter = function(s)
		self:OnSliderChanged("RR", self.Sliders.RR.Wang:GetValue())
		self.Sliders.RR.Slider:InvalidateLayout()
		oldEnter(s)
	end

	self.Sliders.RF = self:NumSlider("Rotate FORWARD", nil, -360, 360, 0)

	self.Sliders.RF.OnValueChanged = function(s, v)
		self:OnSliderChanged("RF", v)
	end

	self.Sliders.RF.Label:SetTextColor(Color(0, 0, 255, 255))

	self.Sliders.RF.Wang.OnEnter = function(s)
		self:OnSliderChanged("RF", self.Sliders.RF.Wang:GetValue())
		self.Sliders.RF.Slider:InvalidateLayout()
		oldEnter(s)
	end
...
function SLIDERS:SetFrameData()
	...
	if not IsValid(selectedFrame) or not selectedBone or not selectedFrame:GetData().BoneInfo[selectedBone] then
...
function SLIDERS:OnSliderChanged(moveType, value)
	if not IsValid(selectedFrame) or not table.HasValue(boneList[selectedBoneSet], selectedBone) then return end --no keyframe/bone selected
...
function SUBANIMS:Init()
	...
	self.AddButton.DoClick = function()
		timeLine:LoadSubAnimation(self.SelectedAnim, self.AnimList)
...
function SUBANIMS:Refresh()
	...
	for i, v in pairs(GetLuaAnimations()) do
		...
		if i ~= "editortest" and i ~= animName and i ~= "editingAnim" and not string.find(i, "subPosture_") then
