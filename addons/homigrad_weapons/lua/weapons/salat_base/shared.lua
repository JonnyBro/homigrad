...
function SWEP:Initialize()
	...
	if SERVER then
		self:SetNWString("skin", table.Random(skins))
	end
...
if SERVER then
	util.AddNetworkString("salat_base_sound")
elseif CLIENT then
	net.Receive("salat_base_sound", function(len)
...
if SERVER then
	util.AddNetworkString("salat_base_shoot")
elseif CLIENT then
	net.Receive("salat_base_shoot", function(len)
...
function SWEP:Deploy()
	...
	if SERVER then
		self:GetOwner():EmitSound("snd_jack_hmcd_pistoldraw.wav", 65, (self.TwoHands and 100) or (not self.TwoHands and 110), 1, CHAN_AUTO)
	end
